...
var (
	background = NewContext(nil, "", nil, nil, nil, nil, nil)
)
...
func (ctx *ctxValue) considerPendingTags() {
	...
	if ctx.loggerInstance != nil {
		ctx.loggerInstance = ctx.loggerInstance.WithFields(pendingTags)
	}
	if ctx.tracerInstance != nil {
		ctx.tracerInstance = ctx.tracerInstance.WithFields(pendingTags)
	}
	if ctx.metricsInstance != nil {
		ctx.metricsInstance = ctx.metricsInstance.WithTags(pendingTags)
	}
}
...
func (ctx *ctxValue) considerPendingFields() {
	...
	if ctx.loggerInstance != nil {
		ctx.loggerInstance = ctx.loggerInstance.WithFields(pendingFields)
	}
	if ctx.tracerInstance != nil {
		ctx.tracerInstance = ctx.tracerInstance.WithFields(pendingFields)
	}
}
