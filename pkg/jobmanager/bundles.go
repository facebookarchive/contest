...
// newStepsBundles creates bundles for test and cleanup steps
func newStepsBundles(descriptors test.StepsDescriptors, registry *pluginregistry.PluginRegistry) ([]test.StepBundle, []test.StepBundle, error) {

	testStepBundles, err := newBundlesFromSteps(descriptors.Test, registry)
	if err != nil {
		return nil, nil, fmt.Errorf("could not create test steps bundle: %v", err)
	}
	cleanupStepBundles, err := newBundlesFromSteps(descriptors.Cleanup, registry)
	if err != nil {
		return nil, nil, fmt.Errorf("could not create cleanup steps bundle: %v", err)
	}
